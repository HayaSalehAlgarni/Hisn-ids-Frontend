...
export default function Settings() {
  ...
  const t = lang === 'ar' ? {
    ...
    reset: {
      ...
      smtpAuthFailed:
        'رفض Gmail تسجيل الدخول (كلمة مرور التطبيق غير صحيحة أو لحساب آخر). فعّل التحقق بخطوتين، أنشئ App Password جديد من نفس حساب SMTP_USER، وضعه في SMTP_PASSWORD في backend/.env ثم أعد تشغيل الخادم.',
      smtpConnectFailed:
        'تعذّر الاتصال بخادم SMTP. تحقق من الشبكة، جدار الحماية، وصحة SMTP_HOST/SMTP_PORT.',
    ...
    reset: {
      ...
      smtpAuthFailed:
        'Gmail rejected the login (wrong App Password or wrong Google account). Enable 2-Step Verification, create a new App Password for the same account as SMTP_USER, put it in SMTP_PASSWORD in backend/.env, then restart the server.',
      smtpConnectFailed:
        'Could not reach the SMTP server. Check network/firewall and SMTP_HOST/SMTP_PORT.',
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      else if (m === 'smtp_auth_failed') setResetError(t.reset.smtpAuthFailed)
      else if (m === 'smtp_connect_failed') setResetError(t.reset.smtpConnectFailed)
